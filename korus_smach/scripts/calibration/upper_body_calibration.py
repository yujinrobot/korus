...
from korus_smach.calibration_tools import actuator
from korus_smach.pick_and_place_tools import misc_tools

class MoveAllToZero(smach.State):
    def __init__(self):
...
#                             output_keys=['goal_pose'])
    ...
    def execute(self, userdata):
        rospy.loginfo('Starting finalisation ...')
        rospy.sleep(0.5)
        rospy.loginfo('Finished finalisation.')
        return 'success'

def main():
    # Initialise the ROS node
    rospy.init_node('upper_body_calibration')
   
    # Create a SMACH state machine
    sm_upper_body = smach.StateMachine(outcomes=['calibrated', 'calibration_failed'])
    # open containers and add states
    with sm_upper_body:
        
        sm_upper_body.userdata.motors = ["torso_turn",
                                         "torso_lift",
                                         "shoulder",
                                         "elbow",
                                         "wrist",
        ...
        smach.StateMachine.add('EnableMotors',
                       ...
                       transitions={'success':'HeadAndTorsoCalibration'})
#                       transitions={'success':'ArmCalibration'})
        
        sm_head_torso = smach.Concurrence(outcomes=['head_and_torso_calibrated', 'head_and_torso_calibration_failed'],
                                          default_outcome='head_and_torso_calibration_failed',
                                          outcome_map={'head_and_torso_calibrated':
                                                       {'HeadCalibration':'head_calibrated',
                                                        'TorsoCalibration':'torso_calibrated'},
                                                       'head_and_torso_calibration_failed':
                                                       {'HeadCalibration':'head_calibration_failed',
                                                        'TorsoCalibration':'torso_calibration_failed'}})
        with sm_head_torso:
            '''
            Head calibration
            '''
            sm_head = head_calibration.createSM()
            smach.Concurrence.add('HeadCalibration', sm_head)
            
            '''
            Torso calibration
            '''
            sm_torso = smach.StateMachine(outcomes=['torso_calibrated', 'torso_calibration_failed'])
            ...
            with sm_torso:
#                smach.StateMachine.add('EnableMotorsTorsoTurn',
#                                       misc_tools.EnableMotors(),
#                                       remapping={'motors':'motor_torso_turn'},
#                                       transitions={'success':'CalibrateTorsoTurn'})
            ...
            smach.Concurrence.add('TorsoCalibration', sm_torso)
            
        smach.StateMachine.add('HeadAndTorsoCalibration', sm_head_torso,
                               transitions={'head_and_torso_calibrated':'ArmCalibration',
                                            'head_and_torso_calibration_failed':'calibration_failed'})
        ...
        sm_arm.userdata.motors_arm_torso_lift = ["torso_lift", "shoulder", "elbow", "wrist", "gripper"]
...
#            smach.StateMachine.add('EnableMotorsArm',
#                                   misc_tools.EnableMotors(),
#                                   remapping={'motors':'motors_arm_torso_lift'},
#                                   transitions={'success':'GetShoulderMaxAndElbowMin'})
            '''
...
#                    sm_wrist_gripper_calibration.userdata.wait_5sec = 5.0
...
#                                                      'actuator_gripper_encoder_centre':'actuator_gripper_encoder_centre'},
            ...
            smach.StateMachine.add('MoveTorsoDownAndElbowToZero',
                                   ...
                                   transitions={'moved':'ResetEncoders',
                                                'error':'arm_calibration_failed'})
            
            smach.StateMachine.add('ResetEncoders',
                                   ServiceState('goo/reset_encoders',
                                                goo_srvs.ResetEncoders,
                                                request = goo_srvs.ResetEncodersRequest("all")),
        ...
        sm_upper_body.userdata.wait_2sec = 2.0
        ...
        sm_upper_body.userdata.arm_default_pose_trajectory = trajectory
    ...
    sis = smach_ros.IntrospectionServer('introspection_server', sm_upper_body, '/SM_ROOT')
    sis.start()
    
    # Execute SMACH plan
    sm_upper_body.execute()
